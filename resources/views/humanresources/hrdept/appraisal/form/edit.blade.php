...
<div class="container">
  ...
  <table width="100%">
    <tr>
      <td>
        ...
        <button type="button" id="myButton" data-bs-toggle="modal" data-bs-target="#section{{ $section->id }}" data-id="{{ $section->id }}">
          {!! $section->section !!}
        </button>
  ...
  <table width="100%">
    ...
    <tr>
      ...
      <td colspan="3" class="td-border-right">
        ...
        <button type="button" id="myButton" data-bs-toggle="modal" data-bs-target="#section_sub{{ $section_sub->id }}" data-id="{{ $section_sub->id }}">
    ...
    <tr>
      ...
      <td colspan="3" class="td-border-right">
        ...
        <button type="button" id="myButton" data-bs-toggle="modal" data-bs-target="#main_question{{ $main_question->id }}" data-id="{{ $main_question->id }}">
    ...
    <tr>
      ...
      <td class="td-border-right">
        ...
        <button type="button" id="myButton" data-bs-toggle="modal" data-bs-target="#question{{ $question->id }}" data-id="{{ $question->id }}">
        {!! $question->question !!}
        </button>
  ...
  <div class="row mb-3">
    <div style="width: 4%">
      <button type="button" data-bs-toggle="modal" data-bs-target="#section_sub_add" data-id="{{ $section->id }}" class="col-auto btn btn-sm btn-outline-secondary">
        <i class="fas fa-plus" aria-hidden="true"></i>
      </button>
    </div>

    <!-- POP UP SECTION SUB -->
    <div class="modal fade" id="section_sub_add" aria-labelledby="sectionsublabeladd" aria-hidden="true">
      <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        {{ Form::open(['route' => ['appraisalform.store'], 'method' => 'POST', 'id' => 'form', 'class' => 'form_section_sub_add', 'autocomplete' => 'off', 'files' => true, 'data-id' => $section->id, 'data-toggle' => 'validator']) }}
        <div class="modal-content">
          <div class="modal-header">
            <h1 class="modal-title fs-5" id="sectionsublabeladd">Appraisal Form :
              {{ $department->department }} Version {{ $pivotappraisal->version }}
            </h1>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body align-items-start justify-content-center">
            <div class="row mb-1">
              <div class="col-sm-2 {{ $errors->has('section_sub_sort_add') ? 'has-error' : '' }}">
                <input type="number" name="section_sub_sort_add" id="section_sub_sort_add" class="form-control form-control-sm" placeholder="Sort" oninput="this.value = (this.value < 1) ? 1 : this.value;">
              </div>
            </div>
            <div class="row mb-1">
              <div class="mb-1 {{ $errors->has('section_sub_text_add') ? 'has-error' : '' }}">
                <textarea name="section_sub_text_add" id="section_sub_text_add" class="form-control form-control-sm"></textarea>
              </div>
            </div>
          </div>
          <div class="modal-footer">
...
(function($) {
  $(document).ready(function() {
    CKEDITOR.replace('section_sub_text_add', {
      toolbar: [{
          name: 'clipboard',
          items: ['Cut', 'Copy', 'Paste', '-', 'Undo', 'Redo']
        },
